...
![image-20220727170952160](E:\JVM\assets\image-20220727170952160.png)
...
![image-20220727171139817](E:\JVM\assets\image-20220727171139817.png)

**程序计数器就是为了记录上面的编号的，java源代码被解释为jvm指令，也就是二进制的字节码文件**

**但是CPU不能执行这些二进制字节码，需要解释器执行为机器码给CPU执行**
...
- 栈内存分配越大越好吗？    
  ...
  答：否，只是可以增加方法的调用个数，线程数反而减少了

  

- 方法内的局部变量是否线程安全？ 

  答：如果方法内局部变量没有逃离方法的作用访问它是线程安全的 

  如果是局部变量引用了对象，并逃离方法的作用范围

  【例如作为参数或者返回值，这样其他的线程可以访问到这个实例】需要考虑线程安全
...
- 这个时候多个线程并发执行，实际上每个线程的栈都会开辟一个栈帧，把a放进去，不是共享的，线程安全

- 如果是a是静态变量 static 修饰，那么两个线程得到的结果，有一个就是3了



#### 1.2.2:栈内存溢出

- 栈帧过多【方法过多】栈内存固定，栈帧过多就会溢出，常见于递归的溢出
...
c或者c++的实现，还有hashcode，wait等方法，其实都是native修饰的本地方法
...
- 有垃圾回收机制【堆里没有引用指向的对象就会被当作垃圾释放】



#### 1.4.1:堆内存溢出

堆内存的对象数量太多，而且都有指向使用，就会导致堆内存溢出
...
配置jvm内存结构的堆大小 VM options : -Xmx1024m

排查堆内存问题，最好设置小一点，更加容易暴露除问题
...
使用jvisualvm可以来连接上去，也能看到内存，线程等信息，有一个强大的功能是堆Dump

可以抓取堆的快照信息和堆内存最大的对象，以及对象的信息等等，就能够知道堆内存到底是被哪个对象消耗了
...
jdk1.6【永久代】===》储存  类的信息Class，类加载器ClassLoader，运行时常量池String Table

![image-20220728110029368](E:\JVM\assets\image-20220728110029368.png)



jdk1.8【元空间，不占用堆，直接使用操作系统内存，StringTable在堆内存里】

![image-20220728110053099](E:\JVM\assets\image-20220728110053099.png)
...
运行时常量池：常量池是 *.class 文件中的，当该类被加载，它在编译时期生成的常量池信息就会放入运行时常量池

并把里面的符号地址变为真实地址，当类加载到内存中后，jvm就会将class常量池中的内容存放到运行时常量池

中，由此可知，运行时常量池也是每个类都有一个

这是刚刚类的常量池信息，如果这个类在运行的话，这些就会被放进运行时常量池，而且运行时不是根据#1，#2

来查找，而是会转化为真实的内存地址来查找
...
可以看到第六行和第29行都是去StringTable里去找#4，六行执行的时候，还没有ab，就会把ab字符串对象放进

StringTable里面，然后第29行再次执行的时候，直接去StringTable找到了"ab"，都是一个对象，所以是true



**对比以上String s4=s1+s2 和 String s5="a"+"b"，为什么一个是false，一个是true，其实我们可以这样**

**想，s1和s2属于变量，是可能改变的值，所以我们不能把他直接放进常量池，而应该创建对象放进堆里面，而**

**String s5="a"+"b"相当于在编译的时候都确定了已经是常量了，就没有必要再创建对象了，直接去找，找到**

**了就直接引用，找不到的话，再创建这个对象放进StringTable**
...
debug模式运行，查看Memory就可以看到StringTable的数量，我们发现只有执行到了对应的代码，才会创建字符串对象存进StringTable里面
...
#### 1.5.11:StringTable位置

在String1.6，StringTable是永久代的一部分

在String1.8，StringTable是堆的一部分
...
- 考虑将字符串对象放入字符串常量池，也就是使用intern把堆内存的String对象放进StringTable，例如你有100万个字符串，全部放入堆内存，就很大，如果这个字符串有很多重复的，我们可以直接使用Intern去重再存入StringTable里面，就能减少很多的内存消耗
...
**磁盘文件读取到系统缓冲区，但是java代码不能读取系统缓冲区的内容，所有需要在java堆内存创建一块缓冲区**

**java代码在这个时候才能读取到java缓冲区的内容，造成了不必要的复制**

![image-20220730092758604](E:\JVM\assets\image-20220730092758604.png)
...
![image-20220730092935536](E:\JVM\assets\image-20220730092935536.png)
...
实际上：直接内存的释放必须主动通过UnSafe对象的方法来管理

ByteBuffer.allocateDirect里面的源码实际上是调用了UnSafe的分配和释放方法
...
一旦 ByteBuffer 对象被垃圾回收，那么就会由 ReferenceHandler 线程通过 Cleaner 的 clean 方法
...
![image-20220730100236078](E:\JVM\assets\image-20220730100236078.png)
...
因为可达性算法的特性，所以可以在gc接触不到的地方作为GC Roots，可以理解为肯定不会被作为垃圾的对象
...
![image-20220730102608682](E:\JVM\assets\image-20220730102608682.png)
...
什么时候A1被回收？只有从GC ROOT到B对象这条线对A1的强引用都断开了，就会被回收，就是C->A1和B->A1



**2：软引用：**

A2对象被GC ROOT软引用了，被B直接引用，假如B对象和A2的强引用关系消失了，只存在了一个软引用，那么A2是可以被回收的，GC ROOT认为不重要，**在内存不足**时可以被回收



**3：弱引用：**

A3对象被GC ROOT软引用了，被B直接引用，假如B对象和A3的强引用关系消失了，只存在了一个弱引用，那么A2是可以被回收的，GC ROOT认为不重要，直接回收



**对比：只存在软引用和弱引用的时候，在内存不足的时候会回收的是软引用，弱引用直接回收**

**软弱引用配合了引用队列的话，他们会进入引用队列，可以对引用队列做进一步处理**
...
![image-20220802155257488](E:\JVM\assets\image-20220802155257488.png)
...
**缺点：容易产生内存碎片，不会对内存再进行整理，例如再来一个数组，就不能插入到上面的任意一个空白位置**

**但是实际上所有白色的位置是足够添加数组的。这就是内存碎片**
...
![image-20220802155931375](E:\JVM\assets\image-20220802155931375.png)
...
![image-20220802160133170](E:\JVM\assets\image-20220802160133170.png)



![image-20220802160533765](E:\JVM\assets\image-20220802160533765.png)
...
<img src="E:\JVM\assets\image-20220802160813734.png" alt="image-20220802160813734"  />
...
![image-20220802162713273](E:\JVM\assets\image-20220802162713273.png)
...
![image-20220803094911621](E:\JVM\assets\image-20220803094911621.png)
...
![image-20220803095614393](E:\JVM\assets\image-20220803095614393.png)
...
![image-20220803100723233](E:\JVM\assets\image-20220803100723233.png)
...
![image-20220823145559402](E:\JVM\assets\image-20220823145559402.png)
...
![image-20220823151702056](E:\JVM\assets\image-20220823151702056.png)
...
![image-20220823152130212](E:\JVM\assets\image-20220823152130212.png)
...
![image-20220823152517202](E:\JVM\assets\image-20220823152517202.png)
...
![image-20220823155928611](E:\JVM\assets\image-20220823155928611.png)



![image-20220823160339362](E:\JVM\assets\image-20220823160339362.png)
...
![image-20220823160544115](E:\JVM\assets\image-20220823160544115.png)
...
![image-20220826084802744](E:\JVM\assets\image-20220826084802744.png)
...
* java的class文件魔数值固定为 0xCAFEBABE，不符合则会抛出错误
...
表明有多少个常量池表项。表项中存放编译时期生成的各种字面量和符号引用
...
仅仅列举了部分，注意：一些比较小的数字，是不会存在常量池的，会直接跟字节码绑定

![image-20220826093312880](E:\JVM\assets\image-20220826093312880.png)



#### 3.4.2:方法字节码载入方法区

![image-20220826093524015](E:\JVM\assets\image-20220826093524015.png)
...
![image-20220826093707260](E:\JVM\assets\image-20220826093707260.png)
...
- 这里小的数字都是和字节码指令存在一起，超过 short 最大值的数字存入了常量池

![image-20220826093848175](E:\JVM\assets\image-20220826093848175.png)
...
![image-20220826093934553](E:\JVM\assets\image-20220826093934553.png)
...
![image-20220826094008264](E:\JVM\assets\image-20220826094008264.png)
...
![image-20220826094043640](E:\JVM\assets\image-20220826094043640.png)



iload_1

![image-20220826094105167](E:\JVM\assets\image-20220826094105167.png)



iload_2

![image-20220826094122511](E:\JVM\assets\image-20220826094122511.png)



iadd

![image-20220826094148201](E:\JVM\assets\image-20220826094148201.png)



istore_3

![image-20220826094217671](E:\JVM\assets\image-20220826094217671.png)



getstatic #4

![image-20220826094243857](E:\JVM\assets\image-20220826094243857.png)

![image-20220826094326460](E:\JVM\assets\image-20220826094326460.png)



iload_3

![image-20220826094353308](E:\JVM\assets\image-20220826094353308.png)
...
![image-20220826094456912](E:\JVM\assets\image-20220826094456912.png)
...
![image-20220826094526863](E:\JVM\assets\image-20220826094526863.png)
...
编译器会按从上至下的顺序，收集所有 static 静态代码块和静态成员赋值的代码

合并为一个特殊的方法<cinit>()V

(cinit)方法会在类加载的初始化阶段被调用
...
所以：执行Test01 d=new Test01("s3",30)并不是直接就执行构造方法了，而是内部组成了新的构造方法
...
- invokespecial可以直接确定绑定了哪个方法，静态的，private的,final的

- invokevirtual在编译的时候，不能确定是调用哪个类的哪个方法，可能是重写的父类的方法，可能是父类的，不确定，所以效率会差点
...
- 比较有意思的是 d.test4() 是通过【对象引用】调用一个静态方法，可以看到在调用 invokestatic 之前执行了 pop 指令，把【对象引用】从操作数栈弹掉了，因为不需要对象
- 还有一个执行 invokespecial 的情况是通过 super 调用父类方法



**所以执行静态方法，不要使用对象.静态方法，会执行对象入栈，然后把对象弹出两条不必要的指令**
...
![image-20220828175419045](E:\JVM\assets\image-20220828175419045.png)
...
![image-20220828175634835](E:\JVM\assets\image-20220828175634835.png)
...
![image-20220828180051166](E:\JVM\assets\image-20220828180051166.png)

![image-20220828180119264](E:\JVM\assets\image-20220828180119264.png)
...
都会在try,catch,catch剩余部分来执行astore的字节码
...
![image-20220828213418162](E:\JVM\assets\image-20220828213418162.png)



- 由于 finally 中的 ireturn 被插入了所有可能的流程，因此返回结果肯定以 finally 的为准 
- 至于字节码中第 2 行，似乎没啥用，且留个伏笔，看下个例子 
- 跟上例中的 finally 相比，发现没有 athrow 了，这告诉我们：如果在 finally 中出现了 return，会 吞掉异常，可以试一下下面
...
![image-20220828213914605](E:\JVM\assets\image-20220828213914605.png)
...
![image-20220829152218114](E:\JVM\assets\image-20220829152218114.png)

![image-20220829152232982](E:\JVM\assets\image-20220829152232982.png)
...
![image-20220829161907539](E:\JVM\assets\image-20220829161907539.png)
...
![image-20220829162028991](E:\JVM\assets\image-20220829162028991.png)



擦除的是字节码上的泛型信息，也就是字节码存储到方法区的信息都是Object类型，但是

LocalVariableTypeTable仍然保留了方法参数泛型的信息

有泛型的，会生成LocalVariableTypeTable表

![image-20220829162208251](E:\JVM\assets\image-20220829162208251.png)
...
**字符串的switch实际上是拆分成两个整数的switch分支，比较String的hashcode和tostring来得到对应的整数值，然后再对这个整数值进行switch来判断执行哪个分支**
...
#### 3.14.9:try-with-resources

JDK 7 开始新增了对需要关闭的资源处理的特殊语法 try-with-resources

以前的话，要自己加上finally来确定资源一定被关闭

只要按照它的规则来写，就可以不写finally
...
**因为在创建 Candy11$1 对象时，将 x 的值赋值给了 Candy11$1 对象的 val属 性** 
...
![image-20220830095854154](E:\JVM\assets\image-20220830095854154.png)

- _java_mirror就是java和instanceKlass之间的桥梁
- 如果这个类还有父类没有加载，先加载父类 
...
![image-20220830100312609](E:\JVM\assets\image-20220830100312609.png)
...
![image-20220830101702709](E:\JVM\assets\image-20220830101702709.png)

**在JDK1.8后，发现静态变量储存在类对象里面**
...
**javap查看，发现static{}里没有putstaic给c和name赋值的情况，但是存在putstaic给obj的情况，表明final修饰的static基本类型，以及字符串常量在初始化的时候就已经赋值了，但是如果是属于引用类型，那么赋值也会在初始化阶段完成**
...
- 首次访问这个类的静态变量或静态方法时，不是final修饰或者是final修饰的引用对象 
...
**会调用invokestatic-->Integer.valueOf**
...
![image-20220830111818009](E:\JVM\assets\image-20220830111818009.png)
...
**Extension使用getParent或者采用Bootstrap类加载器加载的类，使用getClassLoader得到的结果为null，因为是C++的，无法直接访问**
...
- sun.misc.Launcher$ExtClassLoader查看上级为null 委派上级Bootstrap加载类
...
**其实：它最后是使用 Class.forName 完成类的加载和初始化，关联的是应用程序类加载器，因此 可以顺利完成类加载**

**事实上：在某些时候，会打破双亲委派模式，会直接调用Application加载器来完成类加载，不然有些类是找不到的**
...
![image-20220831100914805](E:\JVM\assets\image-20220831100914805.png)
...
![image-20220831105626788](E:\JVM\assets\image-20220831105626788.png)



**中期：耗费时间大大降低**

![image-20220831105723249](E:\JVM\assets\image-20220831105723249.png)



**后期：耗费时间更低**

![image-20220831105755320](E:\JVM\assets\image-20220831105755320.png)
...
0层：字节码通过类加载器加载到JVM，由解释器转变为机器码，让CPU识别，字节码被反复调用达到阈值之后，就会启用编译器来执行编译执行，上升到一层
...
- JIT 是将一些字节码编译为机器码，并存入 Code Cache【缓存】下次遇到相同的代码，直接执行，无需 再编译 
- 解释器是将字节码解释为针对所有平台都通用的机器码【效率较低】 
- JIT 会根据平台类型，生成平台特定的机器码【效率高】
...
刚才的一种优化手段称之为【逃逸分析】发现新建的对象是否逃逸。也就是看我创建的对象是否会在外面被引用，是C2编译器做的优化
...
![image-20220831161232676](E:\JVM\assets\image-20220831161232676.png)
...
![image-20220831161315101](E:\JVM\assets\image-20220831161315101.png)



如果是单线程以上代码是顺序执行（不会交错）就没有问题

![image-20220831161438036](E:\JVM\assets\image-20220831161438036.png)
...
![image-20220831162040526](E:\JVM\assets\image-20220831162040526.png)
...
![image-20220831162541833](E:\JVM\assets\image-20220831162541833.png)
...
![image-20220831163034914](E:\JVM\assets\image-20220831163034914.png)



![image-20220831163101660](E:\JVM\assets\image-20220831163101660.png)
...
![image-20220831164921277](E:\JVM\assets\image-20220831164921277.png)



因为 t 线程要频繁从主内存中读取 run 的值，JIT 编译器会将 run 的值缓存至自己工作内存中的高速缓存中，减少对主存中 run 的访问，提高效率，以致于后面主线程修改了数据，但是t线程还是读到了true

![image-20220831165140944](E:\JVM\assets\image-20220831165140944.png)



1 秒之后，main 线程修改了 run 的值，并同步至主存，而 t 是从自己工作内存中的高速缓存中读 取这个变量的值，结果永远是旧值

![image-20220831165157678](E:\JVM\assets\image-20220831165157678.png)
...
![image-20220831165609089](E:\JVM\assets\image-20220831165609089.png)
...
![image-20220831173523362](E:\JVM\assets\image-20220831173523362.png)
...
![image-20220831180058315](E:\JVM\assets\image-20220831180058315.png)
